...
function render(s){
 ...
 $('signal-title').textContent=s.ollama.online?'Local model is online':'Local model is offline';
 $('signal-copy').textContent='Animated health signal · agent loop not running';
 ...
 $('next-title').textContent=missing.length?'Finish your connections.':'Keep Ollama running.';
 $('next-copy').textContent=missing.length?`Still needed: ${missing.join(', ')}. Credentials belong in the local .env file.`:good?'No account setup is blocking us. Next, connect these decisions to stored memories and real context changes.':'No account setup is blocking us. We’re improving the memory decisions before connecting the full agent loop.';
...
if(toolContext?.registerTool){
 ...
 Promise.resolve(toolContext.registerTool({name:'read_spore_build_status',description:'Read SPORE development milestones, missing setup, and recorded connection checks without spending API credits.',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:{readOnlyHint:true,untrustedContentHint:false},async execute(input){if(!input||typeof input!=='object'||Array.isArray(input)||Object.keys(input).length)throw new Error('Expected an empty object.');await refresh();if(!lastState)throw new Error('Local project status unavailable.');return {focus:lastState.focus,milestones:lastState.milestones,checks:lastState.checks,model:lastState.config.model,ollamaOnline:lastState.ollama.online};}},{signal:lifecycle.signal})).catch(()=>{});
